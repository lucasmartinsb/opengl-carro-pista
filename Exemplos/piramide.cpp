...
const unsigned int MUITOS_CUBOS = 0;
...
int main()
{
    ...
    while (!glfwWindowShouldClose(window))
    {
       ...
       // model = glm::rotate(model, (float)glfwGetTime(), glm::vec3(0.0f, -0.5f, 0.0f));
        ...
        model = glm::rotate(model, (float)glfwGetTime() * velocity, glm::vec3(0.0f, 0.5f, 0.0f));
        view  = glm::translate(view, glm::vec3(0.0f, 0.0f, -3.0f));
        projection = glm::perspective(glm::radians(angulo_visao), (float)SCR_WIDTH / (float)SCR_HEIGHT, near_plane, far_plane);
        ...
        if(MUITOS_CUBOS==1) {
            for(unsigned int i = 0; i < 10; i++)
            {
                ...
                //model = glm::scale(model, glm::vec3(3.0f,1.0f,0.3f));
                model = glm::translate(model, cubePositions[i]);
                float angle = 20.0f * (i + 1) * (i%2==0?1:-1);
                model = glm::rotate(model, (angle * (float) glfwGetTime() * velocity) / 20, glm::vec3(1.0f, 0.3f, 0.5f));
                //model = glm::rotate(model, (angle * (float) glfwGetTime()) / 20, glm::vec3(0.0f, 1.0f, 0.0f));
